...
function setMode(mode){body.dataset.mode=mode;body.classList.toggle('with-ad',mode==='with-ad');buttons.forEach(b=>{const on=b.dataset.mode===mode;b.classList.toggle('active',on);b.setAttribute('aria-pressed',String(on))});syncPremiumHeight()}
...
desktopQuery.addEventListener?.('change',syncPremiumHeight);window.addEventListener('resize',syncPremiumHeight);window.addEventListener('load',syncPremiumHeight);syncPremiumHeight();
